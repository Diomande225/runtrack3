...
async function gererConnexion(evenement) {
    ...
    try {
        ...
        if (reponse.ok) {
            ...
            M.toast({html: 'Connexion réussie ! Vous allez être redirigé vers le calendrier.'});
            console.log('Redirection vers le calendrier dans 2 secondes');
            setTimeout(() => {
                window.location.href = '/page/calendrier.html'; // Assurez-vous que ce chemin est correct
            }, 2000);
        } else {
