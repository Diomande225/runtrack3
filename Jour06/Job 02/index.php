...
$(document).ready(function() {
    // Lien Accueil
    $('a.nav-link:contains("Accueil")').attr('href', 'https://laplateforme.io');

    // Bouton Papillon
    $('.card a.btn-primary').click(function(e) {
        e.preventDefault();
        alert('Vous avez acheté un papillon !');
    });

    // Citations Blade Runner
    ...
    $('.pagination button').click(function() {
        ...
        if (pageNum in pageContent) {
            $('.card-body h2').text(pageContent[pageNum]);
        }
    });
    ...
    // Ajouter des boutons pour contrôler la barre de progression
    $('.progress').before('<button class="btn btn-sm btn-secondary decrease-progress">-</button>');
    $('.progress').after('<button class="btn btn-sm btn-secondary increase-progress">+</button>');
    
    $('.decrease-progress').click(function() {
        if (progress > 0) {
            progress -= 10;
            updateProgressBar();
        }
    ...
    $('.increase-progress').click(function() {
        if (progress < 100) {
            progress += 10;
            updateProgressBar();
        }
    ...
    // Séquence de touches DGC
    let keySequence = '';
    $(document).keydown(function(e) {
        keySequence += String.fromCharCode(e.which);
        if (keySequence.endsWith('DGC')) {
    ...
    $('form').submit(function(e) {
        ...
        if (email && password) {
            ...
            $('.btn-danger').css('background-color', '#' + randomColor);
        }
    });
});
</script>
